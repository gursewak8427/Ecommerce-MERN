...
const Product = ({ productIndex, product }) => {
    ...
    const selectVarient = () => {
                        ...
                        for (let j = 0; j < product.productVarients[i].varienteAttributes.length; j++) {
                            if (product.productVarients[i].varienteAttributes[j].attr_id == attries[m][0]) {
                                if (product.productVarients[i].varienteAttributes[j].value == attries[m][1]) {
                                    data.ids.push(product.productVarients[i]._id)
                                }
                            }
                        }
                    }
        ...
        setSVar(data)
        if ((data.common.length == getVars().length)) {
            // console.log('selected', data.ids[data.ids.length - 1])
            product.productVarients.map(varient => (
                varient._id == data.ids[data.ids.length - 1] ? setSelecctedVarient(varient) : null
            ))
        }
    }
            ...
            <div className="details">
                {
                    product.productType == 0 ? (
                        <div className="pricing">
                            ...
                            <span>{((product.productPricing.mrp - product.productPricing.price) * 100 / product.productPricing.mrp).toFixed(1)}% off</span>
                        </div>
                    ) : product.productType == 1 ? (
                        <div className="pricing">
                            <span>{thisPrice}</span>
