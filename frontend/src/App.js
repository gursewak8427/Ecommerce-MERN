...
import { getTokenUser, getTokenAdmin} from './helpers/auth';
...
function App() {
  ...
  useEffect(()=>{
    if (store.user == "") {
      if (userToken) {
        axios.post(`${KEYS.NODE_URL}/api/user/auth/156/checkUserToken`, { token: userToken })
          .then(result => {
            ...
            console.log('imhere...dfd-err', err)
          })
      }
    }
    ...
    if (store.admin == "") {
      if (adminToken) {
        axios.post(`${KEYS.NODE_URL}/api/vendor/156/checkAdminToken`, { token: adminToken })
          .then(result => {
            console.log(result)
            dispatch({
              type: 'SET_ADMIN',
